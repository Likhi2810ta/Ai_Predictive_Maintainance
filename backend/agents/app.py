...
# backend/ directory (two levels up from agents/app.py)
_BACKEND_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_DATA_DIR     = os.path.join(_BACKEND_ROOT, "data")
_MODELS_DIR   = os.path.join(_BACKEND_ROOT, "models")

BASE_URL    = os.environ.get("LIVE_SERVER_URL", "http://localhost:3000")
...
SIM_DEFAULTS = {
    "CNC_01":      {"temperature_C": 75.0, "vibration_mm_s": 2.5, "rpm": 1480, "current_A": 13.0},
    "CNC_02":      {"temperature_C": 70.0, "vibration_mm_s": 2.0, "rpm": 1490, "current_A": 12.0},
    "PUMP_03":     {"temperature_C": 55.0, "vibration_mm_s": 2.5, "rpm": 2950, "current_A": 18.0},
    "CONVEYOR_04": {"temperature_C": 45.0, "vibration_mm_s": 1.0, "rpm":  720, "current_A":  8.5},
}
SIM_NOISE = {"temperature_C": 3.0, "vibration_mm_s": 0.5, "rpm": 20.0, "current_A": 0.5}
...
def load_assets():
    if not _STATE["baselines"]:
        bl_path = os.path.join(_DATA_DIR, "baselines.json")
    ...
    if not _STATE["models"]:
        for mid in MACHINE_IDS:
            path = os.path.join(_MODELS_DIR, f"model_{mid}.pkl")
            if os.path.isfile(path):
                ...
                _STATE["log_messages"].append(
                    f"model_{mid}.pkl not found — ML detection disabled for {mid}."
                )
...
_PRED_LOG = os.path.join(_DATA_DIR, "predictions_log.csv")
...
def _render_machine_column(machine_id, col, tick):
    with col:
        ...
        st.markdown(f"**📈 Trend Severity: {ts_icon} {pf_score:.0f}% — {w_dir}**")
...
def main():
    ...
    st.caption(
        f"Monitoring: {' · '.join(MACHINE_IDS)}  |  Live Server: {BASE_URL}"
    )

    placeholder       = st.empty()
    maint_placeholder = st.empty()
