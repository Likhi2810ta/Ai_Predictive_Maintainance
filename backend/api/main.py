...
Connects to the Node.js SSE server, runs predictions,
...
# backend/ directory (one level up from api/)
BACKEND_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DATA_DIR     = os.path.join(BACKEND_ROOT, "data")
MODELS_DIR   = os.path.join(BACKEND_ROOT, "models")

NODE_URL    = os.environ.get("LIVE_SERVER_URL", "http://localhost:3000")
...
def _load_assets():
    ...
    bl = os.path.join(DATA_DIR, "baselines.json")
    ...
    for mid in MACHINE_IDS:
        path = os.path.join(MODELS_DIR, f"model_{mid}.pkl")
